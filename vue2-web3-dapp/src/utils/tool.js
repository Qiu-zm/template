...
// bignumber 运算
export function bgfactor(num1, num2){
    let resultnum
    switch (type) {
        case '+':
            resultnum = new BigNumber(num1).plus(new BigNumber(num2))
            break;
        case '-':
            resultnum = new BigNumber(num1).minus(new BigNumber(num2))
            break;
        case '*':
            resultnum = new BigNumber(num1).times(new BigNumber(num2))
            break;
        case '/':
            resultnum = new BigNumber(num1).div(new BigNumber(num2))
            break;
    }
    return resultnum
}
...
export function ConversionAccuracy(value,decimals = 18,type = 'formWei'){
    let num;
    if (type == 'formWei') {
        num = new BigNumber(value).div(new BigNumber(10).pow(decimals))
    }else{
        num = new BigNumber(value).times(new BigNumber(10).pow(decimals))
    }
    return num
}
//导出相应的方法
...
// }
//call
// export function Isallowance(address) {
//     return new Promise((resolve, reject) => {
//         TokenContract.methods.allowance(address, MarketAddress()).call(function (error, result1) {
//             if (!error) {
//                 resolve(result1)
//             } else {
//                 reject(error)
...
// }
//sent
// export function Buyapprove(address) {
//     return new Promise((resolve, reject) => {
//         TokenContract.methods.approve(MarketAddress(), '115792089237316195423570985008687907853269984665640564039457584007913129639935').send({from: address}).on('receipt', function(receipt){
//             resolve(receipt)
...
//MarketContract.methods['getAttributeNFTCount(string,string)'](atrributeType, atrributeValue).call().then(async result1 => {})
//授权所有
// Contract.methods
//         .setApprovalForAll(MarketAddress(), true)
//         .send({ from: that.accountAddr })
...
//             that.$toast.success("Approve Success");
//             setTimeout(() => {
//               that.inif();
//             }, 200);
//           }
...
//           that.isswitch = false;
//           console.error(error);
//           that.$refs.loading.toError();
//         });
//查询授权
// Contract.methods.isApprovedForAll(address, MarketAddress()).call(function (error, result1) {
//     if (!error) {
//         resolve(result1)
//     } else {
//         reject(error)
